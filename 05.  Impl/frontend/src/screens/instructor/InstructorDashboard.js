...
const InstructorDashboard = () => {
  ...
  const handleScanQR = () => {
    ...
    navigation.navigate('InstructorQRScanner', {
      ...
      // You can pass sessionData or other info if needed
    });
  };
  ...
  useEffect(() => {
    ...
    const summary = courses.map(course => ({
      ...
      present: 0, // Set to 0 if no real data
      total: 0,   // Set to 0 if no real data
    }));
    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.tabBarContainer}>
        <TouchableOpacity
          style={[
            styles.tabCard,
            activeTab === 'courses' && styles.tabCardActive,
            { borderTopRightRadius: 0, borderBottomRightRadius: 0 }
          ]}
          onPress={() => setActiveTab('courses')}
        >
          <Ionicons name="book-outline" size={20} color={activeTab === 'courses' ? '#bfa100' : '#22223b'} />
          <Text style={[styles.tabLabel, activeTab === 'courses' && styles.tabLabelActive]}>My Courses</Text>
        </TouchableOpacity>
        <TouchableOpacity
          style={[
            styles.tabCard,
            activeTab === 'attendance' && styles.tabCardActive,
            { borderTopLeftRadius: 0, borderBottomLeftRadius: 0 }
          ]}
          onPress={() => setActiveTab('attendance')}
        >
          <Ionicons name="stats-chart-outline" size={20} color={activeTab === 'attendance' ? '#bfa100' : '#22223b'} />
          <Text
            style={[
              styles.tabLabel,
