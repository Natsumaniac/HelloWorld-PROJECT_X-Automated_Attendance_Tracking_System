...
router.get('/:studentId/attendance-stats', async (req, res) => {
  try {
    const { studentId } = req.params;
    // Find student and their enrolled courses
    const student = await Student.findOne({ idNumber: studentId }).populate('courses');
    if (!student) return res.status(404).json({ error: 'Student not found' });

    // For each course, gather attendance stats
    const stats = await Promise.all(student.courses.map(async (course) => {
