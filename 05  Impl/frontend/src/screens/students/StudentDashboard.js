...
const StudentDashboard = () => {
  ...
  useEffect(() => {
    const fetchCourses = async () => {
      try {
        // Use studentId, not _id, and correct endpoint
        console.log('Fetching courses for:', studentData.idNumber || studentData.studentId);
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={{ alignItems: 'center', marginVertical: 16 }}>
        <Text style={{ fontWeight: 'bold', color: '#bfa100', marginBottom: 8 }}>
          Your Attendance QR Code
        </Text>
        <QRCode
          value={studentData.idNumber || 'NO_ID'}
          size={180}
          backgroundColor="#fff"
        />
        <Text style={{ marginTop: 8, color: '#22223b' }}>
          Student ID: {studentData.idNumber}
        </Text>
      </View>
